...
@section('content')
	<table class="table table-condensed table-bordered">
		<h1>Tours list <span class="small"><a href="{{ route('learningtour::tours.create') }}">+ add tour</a></span></h1>
		@foreach($tours as $tour)
			<tbody class="tour-block">
				<tr>
					<td>{{ $tour->name }}</td>
					<td width="10%">
						<a href="{{ route('learningtour::tours.edit', $tour->id) }}" class="btn btn-xs btn-link">Edit</a>
					</td>
					<td width="5%"><a href="#" class="tour-details-open"><i class="glyphicon glyphicon-menu-down"></i></a></td>
				</tr>
				<tr class="tour-details hidden">
					<td colspan="3">
						<a href="{{ route('learningtour::tours.create-step', $tour->id) }}" class="pull-right">+ add step</a>
						<table class="table table-condensed table-bordered">
									...
									<td width="18%">
										<form action="{{ route('learningtour::tours.delete-step', $step->id) }}" class="submit-delete" method="post">
											...
											<div class="btn-group text-center">
												<a href="{{ route('learningtour::tours.edit-step', $step->id) }}" class="btn btn-xs btn-link">Edit</a>
